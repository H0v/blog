...
class Signup extends React.Component {
  ...
  checkPass = () => {
    const x = this.state.password1 === this.state.password2 ? true : false;
    return x;
  };
    ...
    if (this.checkPass() && this.state.name !== "" && this.state.name !== " ") {
      this.setState(
        state => ({
          ...
          users: [
            ...state.users,
            { name: state.name, password: state.password1, online: true }
          ]
        }),
        () => {
        ...
        <Card className="loginCard" style={this.cardStyle()} variant="outlined">
          <CardContent>
            ...
            <Input
              type="password"
              required={true}
              onChange={event => this.handlePass1(event)}
              className="enterInfo"
            ...
            <Input
              type="password"
              required={true}
              onChange={event => this.handlePass2(event)}
              className="enterInfo"
            ...
            <Typography>
              ...
              <a href="#" onClick={this.props.returnLogin}>
                Log In
              </a>
            </Typography>
          </CardContent>
